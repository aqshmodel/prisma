...
const icons = [
    <HelpCircle key="help" className="w-8 h-8 transition-colors" />,
    <BatteryWarning key="battery" className="w-8 h-8 transition-colors" />,
    <Frown key="frown" className="w-8 h-8 transition-colors" />,
];

export const ProblemSection: React.FC = () => {
    const locale = useLocale();
    const t = getWelcomeText(locale).problem;
            ...
            <div className="max-w-7xl mx-auto px-6 relative z-10">
                <div className="text-left md:text-center max-w-3xl mx-auto mb-12 md:mb-20 space-y-6 animate-fade-in-up">
                    ...
                    <h2 className="text-3xl md:text-5xl font-serif font-medium text-slate-900 leading-[1.4] md:leading-[1.3] tracking-tight text-balance">
                        <span className="inline-block">{t.heading[0]}</span><br className="hidden md:block" />
                        <span className="inline-block">
                            {locale === 'ja' ? (
                                <>その<span className="text-prisma-600">生きづらさ</span>の理由。</>
                            ) : (
                                <>{t.heading[1]}</>
                            )}
                        </span>
                    ...
                    <p className="text-slate-600 text-base md:text-lg leading-[1.8] md:leading-loose tracking-wide font-medium">
                        <span className="inline-block">{t.subheading[0]}</span><br className="hidden md:block" />
                        <span className="inline-block">
                            {locale === 'ja' ? (
                                <>それは、あなたと世界の<span className="text-slate-900 font-bold border-b-2 border-prisma-300/50 inline-block leading-snug">「認知のズレ」</span>が原因かもしれないのです。</>
                            ) : (
                                <>{t.subheading[1]}</>
                            )}
                        </span>
                ...
                <div className="grid md:grid-cols-3 gap-8 mb-20">
                    {t.cards.map((card, idx) => (
                        <div key={card.title} className="group relative h-full" style={{ animationDelay: `${idx * 150}ms` }}>
                            <div className="h-full glass-panel rounded-apple p-8 md:p-10 flex flex-col items-start text-left md:items-center md:text-center transition-all duration-500 ease-apple hover:scale-[1.02] hover:shadow-lg hover:shadow-slate-200/50">
                                ...
                                <h3 className="text-xl font-serif font-bold text-slate-800 mb-4">{card.title}</h3>
                        ...
                        <div className="text-left md:text-center">
                            ...
                            <h3 className="text-xl md:text-2xl font-serif text-slate-800 leading-relaxed">
                                {locale === 'ja' ? (
                                    ...
                                    <>
                                        <span className="inline-block">{t.insight[0]}</span><br className="hidden md:block" />
                                        <span className="inline-block">{t.insight[1]}</span><br className="hidden md:block" />
                                        <span className="inline-block">{t.insight[2]}</span>
                                    </>
                                )}
                            </h3>
                        </div>
